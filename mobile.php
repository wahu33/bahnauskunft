<?php
  ...
  $strDefaultTime = ($arrDate['hours']<=12) ? 7 : 17;
  $strStation = (!empty($_POST['station'])) ? $_POST['station'] : $strDefaultStation ;
  $numHour =  (!empty($_POST['hour'])) ?  (int)$_POST['hour'] : $strDefaultTime;
?>
<!doctype html>
<html>
  <head>
...
<body>
  ...
  <script>
    $(document).ready(function(){
      var datatable=  $('#abfahrttafel').DataTable( {
           "order": [[ 1, "asc" ]],
           "paging":   false,
       });
       datatable.columns( [2,4,5,7] ).visible( false );
    });
  </script>
